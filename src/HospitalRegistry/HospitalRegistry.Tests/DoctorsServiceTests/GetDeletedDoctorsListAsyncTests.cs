...
public class GetDeletedDoctorsListAsyncTests : DoctorsServiceTestsBase
{
    ...
    [InlineData(1, 2)]
    [InlineData(2, 3)]
    public async Task GetDeletedDoctorsListAsync_ReturnsAllDoctors(int pageSize, int pageNumber)
    {
        // Arrange
        ...
        var doctors = GetTestDoctors(isDeleted: true).ToList();
        ...
        // Act
        var actual = await service.GetDeletedDoctorsListAsync(specifications);
